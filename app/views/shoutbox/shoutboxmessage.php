...
while ($row = $result->fetch(PDO::FETCH_LAZY)) {
    ...
    //echo "&nbsp<a href='" . URLROOT . "/shoutbox?reply=" . $row['msgid'] . "' style='font-size: 12px'>[R]</a>";
    if ($_SESSION['class'] > _UPLOADER) {
        ...
        if ($ts + 300 > $now) {
            echo "&nbsp<a href='" . URLROOT . "/shoutbox?quickedit=" . $row['msgid'] . "' style='font-size: 12px'>[E]</a>";
        }
    }
