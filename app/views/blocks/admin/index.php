...
while ($blocks = $data['enabled']->fetch(PDO::FETCH_LAZY)) { ?>
    ...
    <td>[<a href="<?php echo URLROOT ?>/adminblocks/preview?name=<?php echo $blocks["name"] ?>#<?php echo $blocks["name"] ?>" target="_blank">preview</a>]</td>
    </tr>
<?php } ?>
</tbody></table>
...
<table class='table table-striped table-bordered table-hover'><thead><tr>
    ...
    while ($blocks = $data['disabled']->fetch(PDO::FETCH_LAZY)) {
        print("<tr>" .
            ...
            "<td>" . $blocks["sort"] . "</td>" .
            "<td>[<a href=\"".URLROOT."/adminblocks/preview?name=" . $blocks["name"] . "#" . $blocks["name"] . "\" target=\"_blank\">preview</a>]</td>" .
            "</tr>");
    }
    print("<tr><td colspan=\"5\" align=\"center\" valign=\"bottom\" class=\"table_head\"><form action='".URLROOT."/adminblocks/upload'><input type='submit' class='btn btn-sm ttbtn' value='Upload new Block' /></form></td></tr>");
    print("</tbody></table>");
    print("</td></tr></table>");
